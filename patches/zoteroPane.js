...
var ZoteroPane = new function () {
  ...
  /**
   * Batch AI Summary: process selected items sequentially
   * Opens each PDF in a reader tab, triggers the summary flow, waits for completion, moves to next
   */
  this.batchAISummary = async function () {
    var items = this.getSelectedItems();
    if (!items || items.length === 0) return;

    // Filter to items that have PDF attachments
    var pdfItems = [];
    for (let item of items) {
      ...
      if (item.isAttachment() && item.attachmentContentType === 'application/pdf') {
        ...
        if (bestAttachment && bestAttachment.attachmentContentType === 'application/pdf') {
          attachmentID = bestAttachment.id;
        }
      }
      if (attachmentID) {
        pdfItems.push({ item, attachmentID });
      }
    }

    if (pdfItems.length === 0) {
      Services.prompt.alert(null, 'Batch AI Summary', 'No PDF attachments found in selected items.');
      return;
    }
    ...
    let confirmed = Services.prompt.confirm(
      null,
      'Batch AI Summary',
      `Process ${pdfItems.length} paper(s) with AI summary?\n\nThis will open each paper, generate summaries, and close it. This may take a while.`
    );
    if (!confirmed) return;

    Zotero.debug(`[BatchAI] Starting batch processing of ${pdfItems.length} items`);

    let processed = 0;
    let failed = 0;
    let skipped = 0;

    for (let { item, attachmentID } of pdfItems) {
      let title = item.getField('title') || `Item ${item.id}`;
      Zotero.debug(`[BatchAI] (${processed + failed + skipped + 1}/${pdfItems.length}) Processing: ${title}`);

      try {
        // Check if already has summary in VibeDB
        if (Zotero.VibeDB) {
          let existing = await Zotero.VibeDB.getPaper(attachmentID);
          if (existing && existing.summary_cards) {
            Zotero.debug(`[BatchAI] Skipping (already has summary): ${title}`);
            skipped++;
            continue;
          }
        }

        // Open the item in a reader tab
        let reader = await Zotero.Reader.open(attachmentID);
        if (!reader) {
          Zotero.debug(`[BatchAI] Failed to open reader for: ${title}`);
          failed++;
          continue;
        }

        // Wait for reader to fully initialize
        await new Promise(resolve => setTimeout(resolve, 3000));

        // Get the internal reader instance and trigger the flow
        let internalReader = reader._internalReader;
        if (!internalReader) {
          // Try waiting a bit more for internal reader
          for (let i = 0; i < 10; i++) {
            await new Promise(resolve => setTimeout(resolve, 1000));
            internalReader = reader._internalReader;
            if (internalReader) break;
          }
        }

        if (internalReader && internalReader._iframeWindow) {
          // Trigger the flow programmatically via the reader's onflow mechanism
          let flowPromise = new Promise((resolve, reject) => {
            // Watch for flow completion via state changes
            let checkInterval = setInterval(() => {
              try {
                let state = internalReader._state || {};
                let progress = state.flowProgress;
                if (progress) {
                  if (progress.status === 'done' || progress.percent >= 100) {
                    ...
                    clearInterval(checkInterval);
                    reject(new Error(progress.message || 'Flow error'));
                  }
                }
              } catch (e) {
            ...
            // Timeout after 15 minutes per paper
            setTimeout(() => {
              clearInterval(checkInterval);
          ...
          // Click the flow button programmatically
          try {
            let iframe = internalReader._iframeWindow;
            // Dispatch a custom event that the reader listens for
            if (iframe && iframe.document) {
              let btn = iframe.document.querySelector('[class*="flow-button"], [class*="onflow"], [data-action="startFlow"]');
              if (btn) {
                btn.click();
              } else {
                // Try triggering via the reader's internal API
                Zotero.debug(`[BatchAI] No flow button found, trying internal API for: ${title}`);
                // The _executeReparse is on the reader's internal object in the iframe
                // We'll skip this paper if we can't trigger the flow
                failed++;
                // Close the reader tab
                let tabID = Zotero.Reader.getByTabID(Zotero_Tabs.selectedID) === reader ? Zotero_Tabs.selectedID : null;
                if (tabID) Zotero_Tabs.close(tabID);
                continue;
              }
            }
          } catch (e) {
            Zotero.debug(`[BatchAI] Error triggering flow: ${e}`);
          }
          ...
          try {
            await flowPromise;
            processed++;
            Zotero.debug(`[BatchAI] Completed: ${title}`);
          } catch (e) {
            Zotero.debug(`[BatchAI] Failed: ${title} - ${e.message}`);
            failed++;
          }
        } else {
          Zotero.debug(`[BatchAI] Could not get internal reader for: ${title}`);
          failed++;
        }

        // Close the reader tab and wait a moment
        await new Promise(resolve => setTimeout(resolve, 1000));
        let tabID = Zotero_Tabs.selectedID;
        if (tabID && Zotero_Tabs.selectedType === 'reader') {
          Zotero_Tabs.close(tabID);
        }
        await new Promise(resolve => setTimeout(resolve, 2000));

      } catch (e) {
        Zotero.debug(`[BatchAI] Error processing ${title}: ${e}`);
        failed++;
      }
    }
    ...
    Services.prompt.alert(
      null,
      'Batch AI Summary Complete',
      `Processed: ${processed}\nSkipped (already summarized): ${skipped}\nFailed: ${failed}\nTotal: ${pdfItems.length}`
    );
  };
